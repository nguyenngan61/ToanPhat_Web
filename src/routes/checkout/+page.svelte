<script lang="ts">
    ...
    function selectCoupon(c: any) { couponCode = c.code; appliedDiscount = c.discountPercent; showCouponList = false; }
    ...
    <div class="w-[90%] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-4 flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-5">
                <div class="relative">
                    ...
                    <!-- svelte-ignore a11y_label_has_associated_control -->
                    <label class="text-xs text-gray-500 ml-3 bg-white px-1 absolute -top-2 left-0 z-10">Họ và tên</label>
                    <input type="text" class="w-full border border-gray-300 rounded-md px-4 py-3 focus:outline-none focus:border-[#0E3A6B] bg-transparent relative z-0" />
                </div>
                <div class="relative flex">
                    <!-- svelte-ignore a11y_label_has_associated_control -->
                    <label class="text-xs text-gray-500 ml-3 bg-white px-1 absolute -top-2 left-0 z-10">Số điện thoại</label>
                    ...
                    <input type="text" class="w-full border border-gray-300 rounded-r-md px-4 py-3 focus:outline-none focus:border-[#0E3A6B] relative z-0" />
                </div>
                <div class="relative">
                    <!-- svelte-ignore a11y_label_has_associated_control -->
                    <label class="text-xs text-gray-500 ml-3 bg-white px-1 absolute -top-2 left-0 z-10">Tỉnh thành</label>
                    <div class="relative">
                ...
                <div class="relative">
                    <!-- svelte-ignore a11y_label_has_associated_control -->
                    <label class="text-xs text-gray-500 ml-3 bg-white px-1 absolute -top-2 left-0 z-10">Quận huyện</label>
                    <div class="relative">
                ...
                <div class="relative">
                    <!-- svelte-ignore a11y_label_has_associated_control -->
                    <label class="text-xs text-gray-500 ml-3 bg-white px-1 absolute -top-2 left-0 z-10">Phường xã</label>
                    <div class="relative">
                ...
                <div class="relative">
                    ...
                    <textarea class="w-full border border-gray-300 rounded-md px-4 py-3 h-24 resize-none focus:outline-none focus:border-[#0E3A6B] relative z-0"></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-4 flex flex-col gap-8">
            <div>
                ...
                <div class="flex flex-col gap-4">
                    <label class="flex items-center justify-between cursor-pointer p-4 rounded-md border transition-all {shippingFee === 35000 ? 'border-[#0E3A6B] bg-[#0E3A6B]/10 ring-1 ring-[#0E3A6B] text-[#0E3A6B]' : 'border-[#0E3A6B] bg-white text-gray-700 opacity-50 hover:opacity-100'}">
                        <div class="flex items-center gap-3"><input type="radio" name="shipping" value={35000} bind:group={shippingFee} class="w-5 h-5 accent-[#0E3A6B]" /><span class="text-sm font-bold">Tiết kiệm</span></div>
                        <span class="text-sm font-bold">35.000₫</span>
                    </label>
                    <label class="flex items-center justify-between cursor-pointer p-4 rounded-md border transition-all {shippingFee === 50000 ? 'border-[#0E3A6B] bg-[#0E3A6B]/10 ring-1 ring-[#0E3A6B] text-[#0E3A6B]' : 'border-[#0E3A6B] bg-white text-gray-700 opacity-50 hover:opacity-100'}">
                        <div class="flex items-center gap-3"><input type="radio" name="shipping" value={50000} bind:group={shippingFee} class="w-5 h-5 accent-[#0E3A6B]" /><span class="text-sm font-bold">Siêu tốc</span></div>
            ...
            <div>
                ...
                <div class="flex flex-col gap-4">
                    <label class="flex items-center justify-between cursor-pointer p-4 rounded-md border transition-all {paymentMethod === 'cod' ? 'border-[#0E3A6B] bg-[#0E3A6B]/10 ring-1 ring-[#0E3A6B] text-[#0E3A6B]' : 'border-[#0E3A6B] bg-white text-gray-700 opacity-50 hover:opacity-100'}">
                        <div class="flex items-center gap-3"><input type="radio" name="payment" value="cod" bind:group={paymentMethod} class="w-5 h-5 accent-[#0E3A6B]" /><span class="text-sm font-bold">Thanh toán khi nhận hàng (COD)</span></div>
                        <Banknote class="size-5" />
                    </label>
                    <label class="flex items-center justify-between cursor-pointer p-4 rounded-md border transition-all {paymentMethod === 'banking' ? 'border-[#0E3A6B] bg-[#0E3A6B]/10 ring-1 ring-[#0E3A6B] text-[#0E3A6B]' : 'border-[#0E3A6B] bg-white text-gray-700 opacity-50 hover:opacity-100'}">
                        <div class="flex items-center gap-3"><input type="radio" name="payment" value="banking" bind:group={paymentMethod} class="w-5 h-5 accent-[#0E3A6B]" /><span class="text-sm font-bold">Chuyển khoản</span></div>
        ...
        <div class="lg:col-span-4">
            ...
            <div class="border border-gray-200 rounded-md p-4 bg-white shadow-sm relative z-0">
                    ...
                    {#each checkout.items as item}
                        <div class="flex justify-between items-start gap-3">
                            <div class="relative shrink-0">
                                <div class="w-16 h-16 border border-gray-200 rounded bg-white overflow-hidden p-1">
                                    <img src={item.images ? item.images[0] : item.img} alt={item.name} class="w-full h-full object-contain" />
                                </div>
                                </div>
                            <div class="flex-1">
                                <h4 class="text-sm text-gray-700 leading-tight line-clamp-2 font-medium">{item.name}</h4>
                                <div class="flex items-center gap-10 mt-1 text-xs text-gray-500">
                                    <span class="text-gray-400">Toàn Phát</span>
                                    <span class="w-1px h-3 bg-gray-300"></span>
                                    <span>Số lượng: <strong class="text-[#0E3A6B]">{item.quantity}</strong></span>
                                </div>
                            </div>
                            <div class="text-sm font-bold text-[#0E3A6B]">{formatPrice(item.price * item.quantity)}</div>
                        </div>
                    {/each}
                </div>
                ...
                <div class="flex justify-between items-center mt-2 relative z-0"><a href="/cart" class="text-[#00AEEF] text-sm flex items-center gap-1 hover:underline"><ChevronLeft class="size-4" /> Quay về Giỏ hàng</a>
                <button onclick={() => goto('/success')} class="bg-linear-to-r from-[#001E3C] to-[#00AEEF] text-white font-bold px-8 py-3 rounded hover:brightness-110 transition-all shadow-md">Đặt hàng</button></div>
            </div>
        </div>
